...
class PythonAPI(QObject):
    ...
    def selectFolderForBatch(self):
        logging.info("Select folder for batch")
        folder = QFileDialog.getExistingDirectory(None, "Select Folder for Batch Conversion")
        if not folder:
            self._statusText = "No folder selected"
        else:
            self.batchConvert(QUrl.fromLocalFile(folder).toString())
        self.statusTextChanged.emit()
